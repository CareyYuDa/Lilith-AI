...
def ask_api_assistant(question: str, context: str = "") -> str:
    ...
    # 构建 API 模型的完整上下文
    api_llm = ChatOpenAI(
        model=sys_os.getenv("DEEPSEEK_MODEL", "deepseek-v4-flash"),
        api_key=sys_os.getenv("DEEPSEEK_API_KEY"),
        base_url=sys_os.getenv("DEEPSEEK_BASE_URL", "https://opencode.ai/zen/go/v1"),
        temperature=0.7,
        max_tokens=2048,
    )
